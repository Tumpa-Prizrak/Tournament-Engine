...
namespace ChessChallenge.Application
{
    public static class MenuUI
    {
        ...
        {
{ "Valibot", ChallengeController.PlayerType.Valibot },
{ "KurentPosition", ChallengeController.PlayerType.KurentPosition },
{ "KingGambot", ChallengeController.PlayerType.KingGambot },
...
{ "Ace", ChallengeController.PlayerType.Ace },
        ...
        public static void DrawButtons(ChallengeController controller)
        {
            ...
            {
                DrawSettingsButtons(controller);
            } else {
                DrawRegularButtons(controller);
            }
        }

        public static void DrawSettingsButtons(ChallengeController controller)
        {
            ...
            if (NextButtonInRow("Standart", ref buttonPos, spacing, buttonSize, controller.key != "standart"))
            {
                controller.key = "standart";
            }

            if (NextButtonInRow("Chess960", ref buttonPos, spacing, buttonSize, controller.key != "chess960"))
            {
                controller.key = "chess960";
            }

            buttonPos.Y += breakSpacing;

            if (NextButtonInRow("20", ref buttonPos, spacing, buttonSize, controller.GamesCount != 10))
            {
                controller.GamesCount = 10;
            }

            if (NextButtonInRow("50", ref buttonPos, spacing, buttonSize, controller.GamesCount != 25))
            {
                controller.GamesCount = 25;
            }

            if (NextButtonInRow("100", ref buttonPos, spacing, buttonSize, controller.GamesCount != 50))
            {
                controller.GamesCount = 50;
            }

            buttonPos.Y += breakSpacing;

            if (NextButtonInRow("<< Back", ref buttonPos, spacing, buttonSize))
            {
                controller.isSettings = false;
            }
        }

            public static void DrawRegularButtons(ChallengeController controller)
            {
                ...
                if (NextButtonInRow("STOP", ref buttonPos, spacing, buttonSize))
                {
                    controller.EndGame(GameResult.DrawByArbiter, false, false);
                }
                ...
                */
                ...
                if (NextButtonInRow("Settings", ref buttonPos, spacing, buttonSize))
                {
            ...
            static bool NextButtonInRow(string name, ref Vector2 pos, float spacingY, Vector2 size, bool is_disabled = false)
            {
            ...
            static void DrawNextText(string text, ref Vector2 pos, float spacingY, Vector2 size)
            {
